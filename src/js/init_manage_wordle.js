...
inverted_wordles.initWordles = function (wordles, options) {
    ...
    fetch("/api/check_deploy", {
        ...
        if (response.status >= 400 && response.status < 600) {
            response.json().then(res => {
                inverted_wordles.reportStatus("Error at checking wordle deploy status: " + res.error, generalStatusElm, true);
            });
        } else {
            response.json().then(res => {
        ...
        error.json().then(err => {
            inverted_wordles.reportStatus("Error at checking wordle deploy status: " + err.error, generalStatusElm, true);
        });
    });
};
